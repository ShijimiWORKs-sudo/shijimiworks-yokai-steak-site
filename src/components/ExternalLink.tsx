...
export function ExternalLink({ href, children, label, tone = "light", className = "" }: Props) {
  ...
  return <Link href={href} aria-label={`${label}（外部サイト）`} title={`${label}：外部サイトを開く`} target="_blank" rel="noopener noreferrer" className={`${classes} hover:-translate-y-0.5`}>
    {children}<span className="ml-1.5" aria-hidden="true">↗</span>
  </Link>;
}
