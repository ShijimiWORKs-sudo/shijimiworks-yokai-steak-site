...
export function ArticleCard({ article }: { article: Article }) {
  ...
  return (
    <Link href={`/articles/${article.slug}`} className={`article-card group flex min-h-72 flex-col rounded-3xl border p-6 transition duration-300 hover:-translate-y-1 hover:shadow-xl ${creative ? "border-amber-900/20 bg-[#fffaf2]" : "border-black/10 bg-white"}`}>
