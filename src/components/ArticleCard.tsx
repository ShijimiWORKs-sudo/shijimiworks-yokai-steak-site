...
export function ArticleCard({ article }: { article: Article }) {
  ...
  return (
    <Link href={`/articles/${article.slug}`} className={`group flex min-h-72 flex-col rounded-3xl border p-6 transition duration-300 hover:-translate-y-1 hover:shadow-xl ${creative ? "border-amber-900/20 bg-[#fffaf2]" : "border-black/10 bg-white"}`}>
      ...
      <h3 className="mt-3 text-xl font-bold text-slate-900 group-hover:text-sky-800">{article.title}</h3>
      ...
      <p className={`mt-6 text-xs font-bold ${creative ? "text-amber-800" : "text-sky-700"}`}>読む →</p>
    </Link>
  );
}
